...
# Clase de asignación de tareas
class TaskAssignment:

 # Inicialización de clase, los parámetros de entrada requieren una matriz de tareas y métodos de asignación, de los cuales hay dos métodos de asignación, el método de permutación completa all_permutation o el método húngaro Hungría.
 ...
 # Dibuje una línea que cubra 0 y obtenga la matriz con la fila menos el valor mínimo y la columna más el valor mínimo
            delete_count_of_row = []
            delete_count_of_rol = []
 ...
 #Cuando el número de líneas es igual a la longitud de la matriz, salta.
                if line_count == len(b):
                    break
 # Determine si dibuja una línea que cubra todos los ceros y, si está cubierta, realice operaciones de suma y resta
                if 0 not in c:
